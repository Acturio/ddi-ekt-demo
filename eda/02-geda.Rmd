...
stats_transacciones <- data %>% 
  ...
  summarise(
    ...
    q10 = quantile(gtics_transacciones, probs = 0.10, na.rm = T),
    q25 = quantile(gtics_transacciones, probs = 0.25, na.rm = T),
    mediana = median(gtics_transacciones, na.rm = T),
    q75 = quantile(gtics_transacciones, probs = 0.75, na.rm = T),
    q90 = quantile(gtics_transacciones, probs = 0.90, na.rm = T),
...
Con el objetivo de que el lector de esta documentación pueda elegir la variable deseada y estudiar su comportamiento, se presenta una aplicación interactiva para explorar de manera univariada las columnas numéricas que son consideradas en el conjunto de datos.
...
Como primera mejora de modelado, se replica la descomposición de tendencia y estacionalidad usando la transformación logarítmica que se mosotró en secciones anteriores. 
...
Se presenta a continuación un par de ejemplos para posteriormente habilitar al usuario a realizar las características que de forma conjunta que sean de su interés.

```{r}
scatter_ploat_ingresos <- ts_full_data %>% 
  as_tibble() %>% 
  ggplot(aes(
    x = log(gtics_ingresos), 
    y = log(gtics_transacciones),
    colour = gtics_sesiones)) +
  geom_point() +
  labs(title = "Comparación de transacciones e ingresos") +
  coord_flip()
```
...
intervalos_2d <- ts_full_data %>% 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(
    title = "Ingresos a través de transacciones",
    x = "Transacciones",
    y = "Ingresos"
  ) +
...
intervalos_2d_inv <- ts_full_data %>% 
  as_tibble() %>% 
  mutate(
    x = cut_interval(gtics_sesiones, n = m1, dig.lab = 1, labels = F), 
    ingresos = gtics_ingresos) %>% 
  group_by(x) %>% 
  summarise(
    n = n(),
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(
    title = "Ingresos a través de sesiones",
    x = "Sesiones",
    y = "Ingresos"
  ) +
  theme(legend.position = "none")

scatter_ploat_ingresos + intervalos_2d
```
...
ts_full_data %>%
  as_tibble() %>% 
  select(-fecha, -ends_with("lead_generation"), -ends_with("messages"), 
         -ends_with("shopping"),-ends_with("máximo")) %>%
